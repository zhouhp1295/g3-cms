<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['content:friendLink:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['content:friendLink:remove']"
        >删除</el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['content:friendLink:edit']"
          >修改</el-button>
          <el-button
            size="mini"
            type="text"
    ...
    <!-- 添加或修改FriendLink配置对话框 -->
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="120px">
        <el-form-item label="友情链接名称" prop="title" required>
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listFriendLink(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
          this.friendLinkList = response.data.rows;
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除友情链接ID为"' + ids + '"的数据项？').then(function() {
    ...
    handleStatusChange(row) {
      ...
      this.$modal.confirm('确认要"' + text + '""' + row.title + '"吗？').then(function() {
        return changeFriendLinkStatus(row.id, row.status);
      }).then(() => {
